...
class SupabaseTemplateStore:
    ...
    def _load_current_template_assets(self, template_row: Dict[str, Any]) -> Tuple[TemplateJson, str]:
        template_id = str(template_row.get("id"))
        report_type_db = str(template_row.get("report_type") or DEFAULT_REPORT_TYPE_DB)
        current_version = int(template_row.get("current_version") or template_row.get("version") or 0)
        status = str(template_row.get("status") or "draft")
        ...
        if current_version == 0:
            draft_template_json, draft_compiled_html = self._load_draft_assets(template_id, report_type_db)
            has_draft_content = bool(draft_compiled_html) or bool(draft_template_json.sections)
            if has_draft_content or status == "draft":
                return draft_template_json, draft_compiled_html

        if current_version >= 0:
            version_row = self.client.get_template_version(template_id, current_version)
            if version_row:
                return self._load_version_assets(version_row, report_type_db)

        version_rows = self.client.list_template_versions(template_id)
        version_rows.sort(key=lambda item: int(item.get("version_number") or 0), reverse=True)
        for version_row in version_rows:
            template_json, compiled_html = self._load_version_assets(version_row, report_type_db)
            if compiled_html or template_json.sections:
                return template_json, compiled_html

        inline_data = template_row.get("data")
        if isinstance(inline_data, dict):
            return _parse_template_json(inline_data.get("templateJson"), report_type_db), str(inline_data.get("compiledJinja") or "")

        return _parse_template_json({}, report_type_db), ""

    def _resolve_current_compiled_html(self, template_row: Dict[str, Any]) -> Optional[str]:
        template_json, compiled_html = self._load_current_template_assets(template_row)
        if template_json:
            try:
    ...
    def get_preview_html(self, template_id: str) -> Optional[str]:
        ...
        if not row:
            return None
        return self._resolve_current_compiled_html(row)
    ...
    def get_published_template_by_name(self, template_name: str) -> Optional[str]:
        ...
        if not rows:
            return None
        return self._resolve_current_compiled_html(rows[0])
