...
def extract_pages(
    ...
    Args:
        input_path: Ruta al archivo PDF fuente
        output_path: Ruta donde guardar el PDF con las páginas extraídas
        pages: Lista de números de página a extraer (1-based)
    
    Returns:
        Ruta absoluta al archivo generado
    
    Example:
        >>> extract_pages("doc.pdf", "portada.pdf", [1])
        >>> extract_pages("doc.pdf", "seleccion.pdf", [1, 5, 10, 15])
    """
    is_valid, message = validate_pdf_file(input_path)
    if not is_valid:
        raise PDFValidationError(message)
    ...
    if not pages:
        raise ValueError("Debe especificar al menos una página")
    ...
    if invalid_pages:
        raise ValueError(
            f"Páginas inválidas: {invalid_pages}. "
            f"El documento tiene {total_pages} páginas."
        )
    
    writer = PdfWriter()
    
    for page_num in pages:
        writer.add_page(reader.pages[page_num - 1])  # Convertir a 0-based
    ...
    logger.info(
        f"✓ Extraídas {len(pages)} páginas: {pages} → {output_path}"
    )
